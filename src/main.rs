...
const BIG_DATA:     u16 = 'd' as u16 + 'b' as u16 * 256;  // (db) List of data segments
...
fn load_cell<R: Read + Seek>(source: &mut R, size: &mut u32) {
    let cell_size = source.read_i32::<LittleEndian>().unwrap();
    assert!((cell_size.abs() & 0x07) == 0);
    *size -= cell_size.abs() as u32;
    let mut key = [0u8; 2];
    source.read_exact(&mut key).unwrap();
    ...
    source.read_exact(&mut _buf).unwrap();
    match cell_magic {
        KEY_NODE => {
